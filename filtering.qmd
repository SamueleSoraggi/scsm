...
Each person works on its own personal folder in `Data/Students_analysis´, which already contains an aligned dataset. There is a notebook called ´filtering.ipynb´ with only the filtering part, which you can use and modify. 

As a reminder, these are the personal folders with the sample folder contained in them
...
1. Open a new session of jupyterlab in the same way as in [the tutorial session](./tutorial.qmd)
